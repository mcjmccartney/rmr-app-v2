...
export async function POST() {
  try {
    ...
    // Find sessions that are exactly 4 days away (the closest sessions outside the 4-day gap)
    const sessionsToTrigger = sessions.filter(session => {
      ...
      // We want sessions that are exactly 4 days away (the boundary)
      // This means they were previously outside the 4-day gap but now need email notifications
      return daysUntilSession === 4;
    });
    
    console.log(`Found ${sessionsToTrigger.length} sessions that are exactly 4 days away and need email notifications`);
    
    if (sessionsToTrigger.length === 0) {
      return NextResponse.json({
        success: true,
        message: 'No sessions found that need email notifications today',
        sessionsProcessed: 0
      });
    }
    ...
    for (const session of sessionsToTrigger) {
      try {
        ...
        console.log(`Triggering scheduled webhook for session ${session.id}:`, webhookData);
        
        // Trigger the session webhook (same as the regular one but with forced email flag)
        ...
        if (response.ok) {
          console.log(`✅ Successfully triggered scheduled webhook for session ${session.id}`);
          results.push({
            sessionId: session.id,
            clientName: `${client.firstName} ${client.lastName}`,
            status: 'success'
          ...
          console.error(`❌ Failed to trigger scheduled webhook for session ${session.id}:`, response.status, response.statusText);
          results.push({
            sessionId: session.id,
            clientName: `${client.firstName} ${client.lastName}`,
            status: 'failed',
